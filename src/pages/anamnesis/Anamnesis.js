    ...
    initData() {
        ...
        AnamnesisApi.getByDoctor(loadState('doctorName')).then((res) => {
            if (res.data.length>0) {
                res.data[0].map((anamnes) => {
                    if (anamnes) {
                        anamnesis.push({
                            id: anamnes.id,
                            patientID: anamnes.patient.firstName+" "+anamnes.patient.surName,
                            status: anamnes.status,
                            created_at: anamnes.createdAt,
                            view: <Link to={{pathname:"/anamnesis/view", anamnesis:anamnes}}><MDBBtn color="pink" size="sm" rounded><MDBIcon icon="low-vision" className="pr-1"/></MDBBtn></Link>,
                            edit: <Link to={{pathname:"/anamnesis/edit", anamnesis:anamnes}}><MDBBtn color="cyan" size="sm" rounded ><MDBIcon far icon="edit" className="pr-1"/></MDBBtn></Link>,
                            delete: <MDBBtn color="dark-green" size="sm" rounded onClick={()=>this.delete(anamnes)}><MDBIcon icon="archive" className="pr-1"/></MDBBtn>
                        })
                    }
